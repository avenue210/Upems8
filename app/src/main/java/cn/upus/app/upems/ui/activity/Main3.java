...
public class Main3 extends BaseActivity {
    ...
    private static final String TAG = Main3.class.getSimpleName();
    ...
    private void initData() {
        if (!TextUtils.isEmpty(MApp.mSp.getString(UserData.DEVKIND)) && MApp.mSp.getString(UserData.DEVKIND).equals(TYPE_26)) {
            /*商务快递柜 寄存更换为帮助按钮*/
            iv4.setImageDrawable(getResources().getDrawable(R.drawable.ic_horizontal_main_bt4));
    ...
    private void devinfo() {
        try {
            ...
            jsonObject.put("compno", MApp.mSp.getString(UserData.COMPNO));
            jsonObject.put("devno", MApp.mSp.getString(UserData.DEVNO));
            //jsonObject.put("devno", "000201");
            WEB_TYPT = TYPE_2;
            LogUtils.e(TAG, "devinfo");
            ...
            switch (WEB_TYPT) {
                ...
                case TYPE_2://获取设备信息 位置号/设备号。。。。。
                    ...
                    if (null != MApp.systemInfoEntity) {
                        try {
                            if (!TextUtils.isEmpty(MApp.systemInfoEntity.getMainpic())) {
